...
export default function Login() {
  ...
  const handleResetSubmit = async (e) => {
    ...
    try {
      await resetPassword(resetUsername, newPassword);
    } catch (err) {
        ...
        <div style={{ width: '100%', maxWidth: '400px', margin: '0 auto' }}>
          ...
          {!requiresReset ? (
            ...
            <form onSubmit={handleLoginSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div>
                <h2 style={{ fontSize: '1.75rem', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '0.5rem', marginTop: 0 }}>Welcome back</h2>
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', margin: 0 }}>Enter your credentials to access your account</p>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                <label style={{ fontSize: '0.875rem', fontWeight: '600', color: 'var(--text-primary)' }}>Username or Email</label>
                <input
                  type="text"
              ...
              <div style={{ textAlign: 'center', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                ...
                <Link to="/sign-up" style={{ color: 'var(--text-primary)', fontWeight: '600', textDecoration: 'none' }}>
                  Sign Up
                </Link>
              </div>
            </form>
          ) : (
            /* Reset / Set Password Form for Clerk Migrated Users */
            <form onSubmit={handleResetSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div>
                <h2 style={{ fontSize: '1.75rem', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '0.5rem', marginTop: 0 }}>Set Password</h2>
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', margin: 0 }}>Your account was migrated from Clerk. Please set a password to continue.</p>
              </div>
              ...
              >
                {loading ? 'Saving Password...' : 'Save & Login'}
              </button>

              <button
                type="button"
                onClick={() => setRequiresReset(false)}
                style={{
                  padding: '0.75rem',
                  borderRadius: '6px',
