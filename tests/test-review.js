...
async function runTests() {
  try {
    ...
    const initialProdA = await db.prepare("SELECT current_stock FROM products WHERE id = 1").first();
    ...
    // Verify DB update
    const updatedItem = await db.prepare("SELECT * FROM order_items WHERE id = ?").bind(targetItem.id).first();
    ...
    const prodA = await db.prepare("SELECT current_stock FROM products WHERE id = 1").first();
    ...
    const initialProdB = await db.prepare("SELECT current_stock FROM products WHERE id = 2").first();
    ...
    const resolvedOrder = await db.prepare("SELECT * FROM orders WHERE id = ?").bind(targetOrder.id).first();
    ...
    const prodB = await db.prepare("SELECT current_stock FROM products WHERE id = 2").first();
    ...
    const lostOrderRecord = await db.prepare("SELECT * FROM orders WHERE order_id = 'ORD-REV-300'").first();

    // Query Model B stock before lost resolution
    const preLostProdB = await db.prepare("SELECT current_stock FROM products WHERE id = 2").first();
    ...
    const prodBLost = await db.prepare("SELECT current_stock FROM products WHERE id = 2").first();
    ...
    const lostMovement = await db.prepare(`
      SELECT * FROM stock_movements 
      WHERE product_id = 2 AND movement_type = 'write_off' 
