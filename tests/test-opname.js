...
async function runTests() {
  ...
  await storageContext.run(store, async () => {
    try {
      ...
      const updatedP1 = await db.products.get(p1.id);
      const updatedP2 = await db.products.get(p2.id);
      ...
      const movementsP1 = (await db.movements.list()).find(m => m.product_id === p1.id && m.reference === `Stock Opname #${opnameId}`);
      const movementsP2 = (await db.movements.list()).find(m => m.product_id === p2.id && m.reference === `Stock Opname #${opnameId}`);
